...
<div class="bg-white dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-800/20 rounded-2xl shadow-md p-5 flex flex-col">
  ...
  {:else}
    <!-- Day names -->
    <div class="grid grid-cols-7 text-center text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">
    ...
    <div class="grid grid-cols-7 text-center gap-y-2 text-sm">
      ...
      {#each daysInMonth as day}
        ...
        >
          <span
            class="z-10 {isToday(day) ? 'font-semibold text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-gray-200'}"
          >
            {day}
          </span>
    ...
    <!-- Legend -->
    <div class="flex items-center justify-center gap-3 mt-3 text-xs text-gray-500 dark:text-gray-400">
